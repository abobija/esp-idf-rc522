...
{
    if(result->bytes.length == 1
       && result->valid_bits == 4
       && result->bytes.ptr[0] != RC522_MIFARE_ACK) {
...
esp_err_t rc522_nxp_get_version(
        const rc522_handle_t rc522,
        const rc522_picc_t *picc,
        rc522_nxp_picc_version_t *out_version
) {
    RC522_CHECK(rc522 == NULL);
    RC522_CHECK(picc == NULL);
...
esp_err_t rc522_nxp_fast_read(
    ...
    rc522_pcd_crc_t crc = {0};
    RC522_RETURN_ON_ERROR(
            rc522_pcd_calculate_crc(rc522, &(rc522_bytes_t){ .ptr = cmd_buffer, .length = 3, }, &crc));
...
esp_err_t rc522_nxp_write(
        ...
        const rc522_picc_t *picc,
        uint8_t address,
        const uint8_t buffer[RC522_NXP_PAGE_SIZE]
) {
    RC522_CHECK(rc522 == NULL);
    RC522_CHECK(picc == NULL);
    ...
    rc522_pcd_crc_t crc = {0};
    RC522_RETURN_ON_ERROR(rc522_pcd_calculate_crc(rc522, &(rc522_bytes_t){
                .ptr = cmd_buffer,
...
esp_err_t rc522_nxp_read_cnt(
    ...
    uint8_t cmd_buffer[5] = { // send(4)/recv(5) buffer
        RC522_NXP_READ_CNT,
        counter_no,
        0, 0, // CRC
...
esp_err_t rc522_nxp_pwd_auth(
        const rc522_handle_t rc522,
        const rc522_picc_t *picc,
        const uint8_t pwd[RC522_NXP_PWD_SIZE],
        const uint8_t pack[RC522_NXP_PACK_SIZE],
    ...
    *out_state = RC522_PICC_STATE_AUTHENTICATED;
    ...
    // practice to verify
    RC522_CHECK_AND_RETURN(
            memcmp(pack, cmd_buffer, RC522_NXP_PACK_SIZE) != 0,
            RC522_ERR_NXP_PACK_MISMATCH 
            );
    *out_state = RC522_PICC_STATE_AUTHENTICATED;
    return ESP_OK;
}

esp_err_t rc522_nxp_read_sig(
        const rc522_handle_t rc522,
        const rc522_picc_t *picc,
